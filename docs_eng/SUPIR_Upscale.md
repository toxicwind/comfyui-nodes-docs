...
The SUPIR_Upscale node is designed to improve the resolution of input images through a complex process, including magnification, colour correction and noise reduction. It performs these tasks using advanced in-depth learning models to ensure that the images produced are of high quality and detailed.

# Input types
## Required
- supir_model
    - The path of the SUPIR model check point is essential for the magnification process, as it contains the pre-training weights required for the proper operation of the model.
    - Comfy dtype: STRING
    - Python dtype: str
- image
    - The input image that you want to zoom in is the main object of the enhancement process. The quality and resolution of the output image depends heavily on this input.
    - Comfy dtype: IMAGE
    - Python dtype: PIL.Image.Image or torch.Tensor
- steps
    - The number of steps to be used in the proliferation process. More steps usually lead to better results, but more time to calculate.
    - Comfy dtype: INT
    - Python dtype: int

# Output types
- upscaled_image
    - The final output of the node is the enlarged image. It represents the top of the magnification process and is the main result of the SUPIR_Upscale node function.
    - Comfy dtype: IMAGE
    - Python dtype: PIL.Image.Image or torch.Tensor
