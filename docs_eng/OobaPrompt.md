...
The OobaPrompt node is designed to generate detailed image descriptions by processing tips and interacting with the AI text to the image model. It enhances input tips through various modes, such as 'prompt','style', 'descriptor', and 'charcter', to guide the AI model to create a specific type of image. The node can also process custom history records and seeds to generate random changes in output.

# Input types
## Required
- input_prompt
    - The input hint is the key parameter of the node because it directly affects the AI model's understanding of the image to be generated. It should be a descriptive text that clearly depicts the image that is needed.
    - Comfy dtype: STRING
    - Python dtype: str
- mode
    - Mode determines the type of node that will be executed for the input hint. Each mode corresponds to the different ways in which the enhanced hint is used to achieve a particular image generation result.
    - Comfy dtype: ['prompt', 'style', 'descriptor', 'character', 'negative', 'custom']
    - Python dtype: str
## Optional
- custom_history
    - A custom history record is used when the user wants to provide a group of individualized tips to influence the AI model. It should point to a file containing historical reminder data.
    - Comfy dtype: STRING
    - Python dtype: str
- seed
    - Seed parameters are important for generating recoverable image sequences. It ensures that the same input tips produce the same output when giving the same feed values.
    - Comfy dtype: INT
    - Python dtype: int

# Output types
- text
    - The output text is the result of an input hint processed through the AI model. It represents the detailed description of the AI model used to generate the image.
    - Comfy dtype: STRING
    - Python dtype: str
