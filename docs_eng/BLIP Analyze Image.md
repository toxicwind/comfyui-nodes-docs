...
WAS_BLIP_Analyze_Image nodes are designed to analyse and interpret image content using the BLIP model. It provides the function of generating headlines and asking images in natural language questions and provides insight into the visual and contextual aspects of the input image.

# Input types
## Required
- image
    - The image parameter is essential for the operation of the node because it is a model analysis to generate a title or an input to answer the question. It directly affects the output and the quality of the analysis.
    - Comfy dtype: IMAGE
    - Python dtype: PIL.Image or torch.Tensor
- mode
    - Model parameters determine the type of analysis that the node will perform on the image. It can be 'caption' to generate a description, or 'interrogate' to answer questions about the contents of the image.
    - Comfy dtype: COMBO['caption', 'interrogate']
    - Python dtype: str
## Optional
- question
    - When the mode is set to 'interrogate', question parameters are used. It specifies the query that the model will try to answer according to the image content. The wording of the question affects the accuracy and relevance of the answer.
    - Comfy dtype: STRING
    - Python dtype: str
- blip_model
    - The blip_model parameter allows the user to provide preloaded BLIP models for nodes, instead of downloading new models. This is more efficient and particularly useful when running nodes on multiple occasions.
    - Comfy dtype: BLIP_MODEL
    - Python dtype: Tuple[torch.nn.Module, str]

# Output types
- output
    - The output parameter represents the result of the node analysis, which can be a description of the title of the image or an answer to the question posed. It summarizes the node's understanding of the contents of the image.
    - Comfy dtype: TEXT
    - Python dtype: str
